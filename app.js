...
function showOverrideModal() {
    ...
    currentPlan.days.forEach((day, index) => {
        ...
        btn.onclick = () => overrideWorkout(index);
...
function checkForSkippedWorkouts() {
    const history = JSON.parse(localStorage.getItem('gymLogHistory_v2') || '[]');
    if (history.length === 0) return { skipped: false };
    ...
    if (diffDays > 1 && diffDays < 7) {
        ...
        for (let i = 1; i < diffDays; i++) {
            ...
            if (skippedDay && skippedDay.exercises.length > 0) {
                return { skipped: true, skippedDayIndex: skippedDayIndex, skippedDayName: skippedDay.name, lastDayName: workoutPlans[activePlanIndex].days[lastDayIndex].name };
            }
        }
    }
...
function checkForInactivity() {
    const history = JSON.parse(localStorage.getItem('gymLogHistory_v2') || '[]');
    if (history.length === 0) return { inactive: false };
    const lastEntry = history[0];
    const lastDate = new Date(lastEntry.isoDate);
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    ...
    const diffDays = Math.ceil((today - lastDate) / (1000 * 60 * 60 * 24));
    if (diffDays > 3) {
        return { inactive: true, days: diffDays };
    }
...
function getProgressionSuggestion(exerciseName) {
    ...
    if (lastEntry) {
        ...
        if (lastEx && lastEx.sets.length > 0) {
            const topSet = lastEx.sets.reduce((a, b) => (a.weight >= b.weight ? a : b));
            if (topSet.reps >= 12 && parseInt(topSet.rpe) <= 8) return { suggestedWeight: topSet.weight + 2.5 };
            if (topSet.reps < 8 && parseInt(topSet.rpe) > 8) return { suggestedWeight: topSet.weight - 2.5 };
...
function openCardioLogForm() {
    hideQuickLogMenu();
    document.getElementById('cardio-log-modal').style.display = 'block';
}
...
function saveQuickLog() {
    ...
    if (!currentWorkoutLog[exerciseName]) {
        currentWorkoutLog[exerciseName] = { sets: [], notes: '', muscleGroup: muscleGroup };
    }
    const estimated1RM = calculate1RM(weight, reps);
    const newSet = { weight: weight, reps: reps, rpe: "-", e1rm: estimated1RM };
    checkAndSavePR(exerciseName, weight, reps);
    currentWorkoutLog[exerciseName].sets.push(newSet);
    if (!workoutStartTime) { startWorkoutTimer(); }
    saveWorkoutToHistory();
...
function showPage(pageName) {
    document.querySelectorAll(".page, .tab-button").forEach(el => el.classList.remove("active"));
...
function renderLoggedSets(exerciseName, cardId) {
    ...
    currentWorkoutLog[exerciseName].sets.forEach((set, setIndex) => {
        ...
        setItem.innerHTML = `<span>Set ${setIndex + 1}: ${set.weight}kg x ${set.reps} reps @RPE ${set.rpe}</span><div class="set-item-details"><span class="estimated-1rm">1RM ≈ ${set.e1rm.toFixed(1)} kg</span><button class="btn-delete" onclick="deleteSet('${exerciseName.replace(/'/g, "\\'")}', '${cardId}', ${setIndex})"><i data-feather="trash-2"></i></button></div>`;
...
function logSet(cardId, exerciseName, weight, reps, rpe, fromRepeat = false) {
    ...
    if (finishBtn.style.display === 'none') {
        ...
        finishBtn.style.animation = 'fadeIn 0.5s';
    }
    ...
    const estimated1RM = calculate1RM(weight, reps);
    const logData = { weight, reps, rpe: rpe || "-", e1rm: estimated1RM };
    
    if (!currentWorkoutLog[exerciseName]) {
        currentWorkoutLog[exerciseName] = { sets: [], notes: '', muscleGroup: 'Other' };
    }
...
function saveWorkoutToHistory() {
    ...
    Object.keys(currentWorkoutLog).forEach((exKey) => {
        ...
        if (exLog && ( (exLog.sets && exLog.sets.length > 0) || exLog.type === 'cardio')) {
            const exCard = Array.from(document.querySelectorAll('.exercise-card')).find(card => card.querySelector('.exercise-title')?.textContent.startsWith(exLog.name));
    ...
    const newEntry = { id: todayId, isoDate: today.toISOString(), dayName: document.getElementById('workout-day-title').textContent, totalVolume: totalVolume, duration: durationStr, exercises: exercisesData, prsAchieved: currentSessionPRs };
    const todayIndex = history.findIndex(entry => entry.id === todayId);
    if (todayIndex > -1) {
...
function renderPRsPage() {
    ...
    sortedPRs.forEach(([exerciseName, prData]) => {
        ...
        prItem.addEventListener('click', () => prItem.classList.toggle('open'));
...
function renderFreestyleLog(exerciseName, setData) {
    ...
    if (!card) {
        ...
        card.id = cardId;
        card.innerHTML = `<div class="ex-header"><div class="ex-title-container"><div class="exercise-title">${exerciseName} (Freestyle)</div></div></div><div id="logged-sets-${cardId}" style="margin-bottom: 15px;"></div>`;
        exerciseList.insertBefore(card, exerciseList.firstChild);
    }
    const loggedSetsContainer = document.getElementById(`logged-sets-${cardId}`);
    const setItem = document.createElement("div");
    setItem.className = 'list-item history-set-item';
    setItem.style.padding = '8px';
    setItem.innerHTML = `<div class="set-item-details"><span>Set ${currentWorkoutLog[exerciseName].sets.length}: ${setData.weight}kg x ${setData.reps} reps</span><span class="estimated-1rm">1RM ≈ ${setData.e1rm.toFixed(1)} kg</span></div><button class="btn-delete" onclick="deleteSet('${exerciseName.replace(/'/g, "\\'")}', '${cardId}', ${currentWorkoutLog[exerciseName].sets.length - 1})"><i data-feather="trash-2"></i></button>`;
    loggedSetsContainer.appendChild(setItem);
    feather.replace();
}
...
function generateCardioCharts() {
    ...
    const cardioHistory = history
        .flatMap(entry => entry.exercises.filter(ex => ex.type === 'cardio').map(ex => ({...ex, date: new Date(entry.isoDate)})))
        .sort((a,b) => a.date - b.date);

    if (cardioHistory.length > 0) {
        const distanceData = {
            labels: cardioHistory.map(c => c.date.toLocaleDateString("th-TH", {month:"short", day:"numeric"})),
            datasets: [
                {
                    label: 'ระยะทาง (กม.)',
        ...
        cardioDistanceChart = new Chart(ctxDistance, { type: 'bar', data: distanceData, options: { scales: { yDistance: { position: 'left', title: {display: true, text: 'กม.'} }, yDuration: { position: 'right', title: {display: true, text: 'นาที'}, grid: {drawOnChartArea: false} } } } });
        ...
        cardioPaceChart = new Chart(ctxPace, { type: "line", data: paceData, options: { scales: {y: { reverse: true, title: {display: true, text: 'Pace (ยิ่งน้อยยิ่งดี)'}}}} });
        ...
        cardioWeeklySummaryChart = new Chart(ctxWeekly, {
            type: 'bar',
            data: { labels: weeklyLabels, datasets: [{ label: 'Total Distance (km)', data: weeklyData, backgroundColor: 'rgba(3, 218, 198, 0.5)', borderColor: 'rgba(3, 218, 198, 1)', borderWidth: 1 }] },
...
function generateWeeklyVolumeChart() {
    ...
    weeklyVolumeChart = new Chart(ctx, {
        type: 'bar',
        data: { labels, datasets: [{ label: 'Total Volume (kg)', data, backgroundColor: 'rgba(3, 218, 198, 0.5)', borderColor: 'rgba(3, 218, 198, 1)', borderWidth: 1 }] },
...
function generateFatigueAnalysisChart() {
    ...
    fatigueAnalysisChart = new Chart(ctx, {
        ...
        data: {
            labels,
            datasets: [
                { label: 'Weekly Volume', data: volumeData, yAxisID: 'yVolume', backgroundColor: 'rgba(3, 218, 198, 0.5)' },
                { 
                    label: 'Avg Max e1RM', 
                    data: avgE1rmData, 
                    yAxisID: 'yE1rm', 
...
function generateComparisonChart() {
    ...
    history.forEach(entry => allLabelsSet.add(new Date(entry.isoDate).toLocaleDateString("th-TH", { month: "short", day: "numeric" })));
...
function generateMuscleBalanceChart() {
    ...
    muscleBalanceChart = new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                r: {
                    angleLines: { color: getThemeColors().borderColor },
                    grid: { color: getThemeColors().borderColor },
                    pointLabels: { color: getThemeColors().textColor, font: { size: 12 } },
                    ticks: { display: false }
                }
            },
...
function generateExerciseCharts(exerciseName) {
    ...
    chartVolume = new Chart(ctxVolume, {
        ...
        options: { plugins: { legend: { display: !1 } }, scales: { y: { beginAtZero: !0 } } }
    });
    ...
    chartStrength = new Chart(ctxStrength, {
        ...
        options: { plugins: { legend: { display: !1 } }, scales: { y: { beginAtZero: !1 } } }
    });
}

function checkAndSavePR(exerciseName, weight, reps) {
    if (!personalRecords[exerciseName]) {
        personalRecords[exerciseName] = { maxWeight: 0, repPRs: {} };
    }
    const prData = personalRecords[exerciseName];
    let message = '';
    ...
    if (weight > prData.maxWeight) {
        ...
        currentSessionPRs.push({type: 'weight', exercise: exerciseName, weight: weight});
    }
    if (!prData.repPRs[weight] || reps > prData.repPRs[weight]) {
        newRepPR = true;
        prData.repPRs[weight] = reps;
        message += `⭐ สถิติใหม่ที่ ${weight}kg: ${reps} ครั้ง!`;
        currentSessionPRs.push({type: 'reps', exercise: exerciseName, weight: weight, reps: reps});
    }
        ...
        entry.exercises.forEach(ex => {
            if (ex.name === exerciseName && ex.type !== 'cardio') {
                ex.sets.forEach(set => {
                    ...
                    if (!newPRData.repPRs[set.weight] || set.reps > newPRData.repPRs[set.weight]) { newPRData.repPRs[set.weight] = set.reps; }
                });
            }
        });
    });
...
function saveBodyStats(){
    ...
    if(hasData) {
        ...
        if(existingIndex > -1) {
            bodyStats[existingIndex] = {...bodyStats[existingIndex], ...newStat};
        } else {
...
function openPlateCalculator(cardId) {
    ...
    document.getElementById('plate-calculator-modal').style.display = 'block';
}
...
function saveEquipmentSettings() {
    ...
    userEquipment = { barbellWeight, availablePlates: plates };
